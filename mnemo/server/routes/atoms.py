...
async def create_atom(agent_id: UUID, body: AtomCreate):
...
async def get_atom(agent_id: UUID, atom_id: UUID):
    async with get_conn() as conn:
        await _require_active_agent(conn, agent_id)
...
async def delete_atom(agent_id: UUID, atom_id: UUID):
    async with get_conn() as conn:
        await _require_active_agent(conn, agent_id)
...
async def link_atoms(agent_id: UUID, body: EdgeCreate):
