...
async def remember(agent_id: UUID, body: RememberRequest, operator=Depends(get_current_operator)):
    ...
    async with get_conn() as conn:
        ...
        await log_operation(
            conn, "remember", operator["id"], target_id=agent_id,
...
async def recall(agent_id: UUID, body: RetrieveRequest, operator=Depends(get_current_operator)):
    ...
    async with get_conn() as conn:
        ...
        await log_operation(
            conn, "recall", operator["id"], target_id=agent_id,
            duration_ms=int((time.monotonic() - t0) * 1000),
