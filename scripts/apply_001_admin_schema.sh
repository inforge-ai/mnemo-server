#!/usr/bin/env bash
# Apply the 001_admin_schema migration to both production and test databases.
# Must be run as a user who can sudo to postgres (or directly as postgres).
#
# Usage:
#   sudo -u postgres bash scripts/apply_001_admin_schema.sh

set -euo pipefail

MIGRATION="migrations/001_admin_schema.sql"
